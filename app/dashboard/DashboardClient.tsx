...
import { Calendar, TrendingUp, Edit3, Lock, Check, X, FlaskConical, AlertCircle, KeyRound, Trophy, MessageCircle, Settings, ChevronRight } from 'lucide-react'
...
export default function DashboardClient({
  ...
  const chickenDinners = teamMatches.filter(m => m.position === 1).length
    ...
    <main className={styles.page}>
      <div className="container">
        ...
        <div className={styles.statsBar}>
          ...
          <div className={styles.statItem}>
            <span className={styles.statValue}>{chickenDinners}</span>
            <span className={styles.statLabel}>CHICKEN DINNERS</span>
          </div>
          <div className={styles.statDivider} />
          <div className={styles.statItem}>
            <span className={styles.statValue}>{upcomingBookings.length + pastBookings.length}</span>
            <span className={styles.statLabel}>SLOTS</span>
          </div>
          <div className={styles.statDivider} />
          <div className={styles.statItem}>
            <span className={styles.statValue}>{leaderboardEntry?.matches_played ?? 0}</span>
            <span className={styles.statLabel}>MATCHES</span>
          </div>
          <div className={styles.statDivider} />
          <div className={styles.statItem}>
            <span className={styles.statValue} style={{ color: rank > 0 ? getRankColor(rank) : '#666' }}>
              {rank > 0 ? '#'+rank : '—'}
            </span>
            <span className={styles.statLabel}>IN RACE</span>
          </div>
        </div>
          ...
          <div className={styles.rankCard}>
            ...
            <div className={styles.rankBody}>
              ...
              <div className={styles.rankStats}>
                <div><span className={styles.rankStatVal}>{chickenDinners}</span><span className={styles.rankStatLabel}>Chicken Dinners</span></div>
                <div><span className={styles.rankStatVal}>{leaderboardEntry?.matches_played ?? 0}</span><span className={styles.rankStatLabel}>Matches</span></div>
                <div><span className={styles.rankStatVal}>{leaderboardEntry?.total_kills ?? 0}</span><span className={styles.rankStatLabel}>Kills</span></div>
              </div>
              <div style={{
